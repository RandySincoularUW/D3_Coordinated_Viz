...
function updateChart(bars, n, colorScale) {
	...
	*/
	...
	if (expressed == "AVG_ELEC") {
		newTitle = "Average Price of Electric";
	} else if (expressed == "ENERGY_CONSUMED") {
		newTitle = "Energy Consumed"
	} else if (expressed == "ENERGY_EXPEND") {
		newTitle = "Energy Expended"
	} else if (expressed == "ENERGY_PROD") {
		newTitle = "Energy Produced" 
	} else {
		newTitle = "Natural Gas Prices"
	};
		
	
	/*  1-apr-17
	var chartTitle = d3.select(".chartTitle")
		.text(expressed + " for each state");
	*/
